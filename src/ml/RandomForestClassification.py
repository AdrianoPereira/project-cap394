...
def rf_classifier(df, threshold=1, records=1000, test_size=.2):
    columns = [
    ...
    dt = RandomForestClassifier(n_estimators=200).fit(X_train, y_train)
...
if __name__ == "__main__":
    ...
    for x in np.arange(10)+1:
        dt, df_, X_test, y_test = rf_classifier(df)
